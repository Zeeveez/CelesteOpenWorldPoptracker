...
raw_locations = load_csv('./locations/locations.csv')
...
for row in original_map_sizes:
    ...
    zoom = 1 / perc_all_screen / 2
    rooms[room_id] += [[map_id, f'{cx},{cy}', f'{zoom}']]
...
with open('./scripts/autotracking/autotabbing/room_tabs.lua','w') as f:
    ...
    f.write(f'\t[\'\'] = {{ {{ \'Summary\' }} }},\n')
    for room in rooms:
        f.write(f'\t[\'{room}\'] = {str(rooms[room]).replace('[', '{ ').replace(']',' }')},\n')
    f.write(f'}}')
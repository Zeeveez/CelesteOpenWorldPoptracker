...
function onClear(slot_data)
    ...
    if slot_data then
        if slot_data['strawberries_required'] then
            local obj = Tracker:FindObjectForCode("strawberries_required")
            local count = slot_data['strawberries_required']
            if obj then
        ...
        if slot_data['include_b_sides'] then
            local obj = Tracker:FindObjectForCode("include_b_sides")
            local active = slot_data['include_b_sides'] == 1
            if obj then
                obj.Active = active
            end
        end
        if slot_data['include_c_sides'] then
            local obj = Tracker:FindObjectForCode("include_c_sides")
            local active = slot_data['include_c_sides'] == 1
            if obj then
                obj.Active = active
            end
        end
        if slot_data['include_core'] then
            local obj = Tracker:FindObjectForCode("include_core")
            local active = slot_data['include_core'] == 1
            if obj then
                obj.Active = active
            end
        end
        ...
        if slot_data['include_goldens'] then
            local obj = Tracker:FindObjectForCode("include_goldens")
            local active = slot_data['include_goldens'] == 1
            if obj then
                obj.Active = active
            end
        end

...
celeste_open_world_location = {}
celeste_open_world_location.__index = celeste_open_world_location
...
function celeste_open_world_location.new(name)
    local self = setmetatable({}, celeste_open_world_location)
    if name then
...
function celeste_open_world_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = celeste_open_world_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function celeste_open_world_location:connect_two_ways(exit, rule)
...
function celeste_open_world_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function celeste_open_world_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function celeste_open_world_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function celeste_open_world_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function celeste_open_world_location:accessibility()
...
-- 
function celeste_open_world_location:discover(accessibility, keys)
...
entry_point = celeste_open_world_location.new("entry_point")
...
ScriptHost:LoadScript("scripts/Logic/roomlogic.lua")